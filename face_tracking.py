    ...
    with dai.Device(pipeline) as device: #起動したカメラ実体のことを、これ以降のプログラムの中では device という名前で呼びますよ
        ...
        while True:
            ...
            if dets.shape[0] > 0:#「検出結果（dets）の中に、1つでも候補があるか？」を確認 numpy配列を入れた変数だから.shapeが使える？
                ...
                keep_idx = np.squeeze(keep_idx)  # [box_num, class_num] -> [box_num] ←このようにNMSBoxes が返すデータの形（次元）を整えます。
                dets = dets[keep_idx]#元々の全ての候補データ（dets）の中から、選ばれた番号（keep_idx）のものだけを抽出して上書きします。
            ...
            if dets.shape[0] > 0:
                ...
                draw(
                    img=frame,
                    bboxes=dets[:, :4],#顔を囲む四角形。
                    landmarks=np.reshape(dets[:, 4:14], (-1, 5, 2)),#目・鼻・口の端の5つの点。ここで reshape しているのは、10個の数字（X1, Y1, X2, Y2...）を「5地点 × XY座標」という形に整理するためです。
                    scores=dets[:, -1],#「自信度」の数字。
                )
